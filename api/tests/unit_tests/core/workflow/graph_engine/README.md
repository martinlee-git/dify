...
작업흐름 그래프 엔진 연기 테스트 이제 이 디렉토리는 외부 그래폰 패키지 주위에 Dify 소유의 작은 연기 레이어만 유지합니다. 유지된 적용 범위는 다음에 중점을 둡니다. 1. 워크플로 레이어 정의: 레이어/테스트 llm quota.py 레이어/테스트 observability.py 2. 인간 입력 재개 통합: 병렬 인간 입력 조인 이력서.py 테스트 3. 하나의 모의 도구/chatflow 스모크 경로: chatflow.py의 테스트 도구 아래 도우미 모듈은 유지된 스모크 테스트에서 사용하기 때문에 남아 있습니다. 1. 모의 config.py 테스트 2. 모의 공장 테스트 3. 모의 노드.py 테스트 4. 테스트 테이블 run.py 예:
